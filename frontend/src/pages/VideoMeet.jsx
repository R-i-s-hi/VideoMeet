...
function VideoMeetComponent() {
      ...
      if (newVideoTrack) {
        ...
        if (videoSender) {
          videoSender
            ...
            .catch((err) => {
              console.error(
                `Failed to replace video track for ${peerId}:`,
                err
              );
            });
        }
      ...
      if (newAudioTrack) {
        ...
        if (audioSender) {
          audioSender
            ...
            .catch((err) => {
              console.error(
                `Failed to replace audio track for ${peerId}:`,
                err
              );
            });
        }
  ...
  const gotMessageFromServer = async (fromId, rawMessage) => {
    ...
    try {
      ...
      const peer =
        connections[fromId] ||
        (await createPeerConnection(fromId, isPolite, peerUsername));
      ...
      if (signal.sdp) {
        ...
        const offerCollision =
          isOffer && (peer.isMakingOffer || peer.signalingState !== "stable");
        ...
        if (offerCollision) {
          ...
          if (!peer.isPolite) {
            console.warn(
              "Offer collision: impolite peer handling offer from",
              fromId
            );
            try {
              ...
              console.log(
                "Impolite peer handled offer collision and responded with answer to",
                fromId
              );
            } catch (err) {
              console.error(
                "Failed to resolve offer collision (impolite):",
                err
              );
        ...
        if (description.type === "answer" && peer.signalingState === "stable") {
          console.warn(
            "Received an answer but we're already stable — ignoring late answer"
          );
          return;
        }
        ...
        try {
          await peer.setRemoteDescription(description);
          console.log(
            `setRemoteDescription(${description.type}) from ${fromId}`
          );
        } catch (e) {
        ...
        try {
          if (peer.remoteDescription && peer.remoteDescription.type) {
            await peer
              ...
              .then(() => console.log("added ICE Candidate: ", signal.ice));
      ...
      if (
        e.name.includes("InvalidStateError") ||
        e.name.includes("InvalidAccessError")
      ) {
  ...
  const connectToSocketServer = () => {
    ...
    socketRef.current.on("connect", () => {
      ...
      socketRef.current.on("existing-users", async (users) => {
        await Promise.all(
          users.map(async ({ id: clientId, username: peerUsername }) => {
      ...
      socketRef.current.on("video-toggle", ({ socketId, enabled }) => {
        ...
        if (socketId === socketIdRef.current) {
          ...
          if (videoTrack && videoTrack.enabled !== enabled) {
            console.warn(
              `State mismatch detected! Forcing track to ${enabled}`
            );
      ...
      socketRef.current.on(
        "user-joined",
        async (id, allClientIds, peerUsername) => {
            ...
            <div className={styles.videoWrapper}>
              ...
              <div
                className={`${styles.conferenceView} ${
                  videos.length >= 4 ? styles.twoRowGrid : ""
                }`}
              >
                {videos.map(
                  ({
